...
const getPoolConfigs = {
	...
	//Address Seperators
    paymentId: config.poolServer.paymentId,
    fixedDiff: config.poolServer.fixedDiff,
...
function handleTop10(response){
	...
	if(
		Object.keys(topMinersCache.miner).length > 0 ||
		Object.keys(topMinersCache.donate).length > 0 ||
		Object.keys(topMinersCache.unblocker).length > 0 
	  ){
		return 	sendData(response,topMinersCache);
	}
...
function handleAdminPorts(response){
    async.waterfall([
        ...
        function(portsKeys, callback) {
            ...
            sendData(response,(error)?{error: 'Error collecting Ports stats'}:portsData);
    });
}
