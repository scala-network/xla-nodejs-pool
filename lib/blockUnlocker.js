        ...
        function(blocks, callback){
            ...
            if (config.poolServer.donations && config.poolServer.donations.enabled) {
                ...
                redisClient.multi(getDonationCommands).exec(function(error, replies) {
                    ...
                    const blockStatisticHeight = Object.keys(blockStatistic);
                    for(let i = 0;il=blockStatisticHeight.length;i<il;i++){
                        const height = blockStatisticHeight[i];
                        const blockStatisticWallets = blockStatistic[height];
			    ...
			    if(donateLevelWallet.hasOwnProperty(wallet)){
                             	donation = Math.floor(stats.earn * (donateLevelWallet[wallet] / 100.));
			    }
                            unlockedBlocksCommands.push(["hmset",config.coin+":block_shares:"+height,wallet,[
                                stats.score,
                                stats.percent,
                                stats.earn,
                                donation,
                ...
                const blockStatisticHeight = Object.keys(blockStatistic);
			for(let i = 0;il=blockStatisticHeight.length;i<l;i++){
			const height = blockStatisticHeight[i];
			const blockStatisticWallets = blockStatistic[height];
			const wallets = Object.keys(blockStatisticWallets);
