 ...
 * Block template
 **/
var BlockTemplate = function (template){
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
        	...
        	var donations;
            if (config.poolServer.donations) {
                var escaped_delimiter = ((config.poolServer.donations.addressDelimiter || '%') + '').replace(
                    /([.\\+*?\[\]^$()])/g, '\\$1');
                login = login.replace(new RegExp(escaped_delimiter + "(\\d+(?:\\.\\d+)?|\\.\\d+)" + escaped_delimiter),
                    function(match, p1) {
                        customDonation = parseFloat(p1);
                        // The amount in the tag includes the pool fee, so subtract it off (and
                        // enforce minimum = pool fee).
                        donations -= config.blockUnlocker.poolFee;
                        if (donations < 0) {
                        	donations = 0.0;
                        }
                        return '';
...
function newConnectedWorker(miner){
    ...
    if (miner.difficulty) {
    	log('info', logSystem, 'Miner difficulty %s to %d', [(miner.noRetarget)?"fixed":"set",miner.difficulty]);
    }
	
	let donation = miner.donation || false;
	
	if (Object.keys(config.donations).length > 0) {
        if (donation) {
            log('info', logSystem, 'Miner donation set to %s via custom donation in login username', [donation]);
        } else {
        	for (let p of config.poolServer.ports) {
                if (p.port !== miner.port || !('donation' in p) || p.donation <= 0.0) {
                    continue;
                }
                donation = p.donation;
            }
            log('info', logSystem, 'Miner donation set to %s (via connection on port %d)', [donation, miner.port]);
        }
...
function processShare(miner, job, blockTemplate, params){
	...
	const convertedBlob = utils.cnUtil.convert_blob(shareBuffer);
    var hash;
    var shareType;
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
    	...
    	const cn_variant = convertedBlob[0] >= 3 ? convertedBlob[0] - 2 : false;
