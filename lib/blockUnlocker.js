...
function runInterval(){
    async.waterfall([
        ...
        function(blocks, callback){
			...
			function delBlockStats(height,wallet,key,value){
				if(!unblockStatsRedis.hasOwnProperty(height)){
					unblockStatsRedis[height] = {};
				}
				if(!unblockStatsRedis[height].hasOwnProperty(wallet)){
            ...
            blocks.forEach(function(block){
                ...
                if (Object.keys(global.config.contributions).length) {
                    for(var wallet in global.config.contributions) {
						...
						addBlockStats(block.height,wallet,"total",pay);
                        reward -=pay;
				...
				let contributionFee =  roundUpPercent(block.reward - reward);
				/* Additional block reward to those who unlocks a block*/
				let unblockerAward = 0.0;
				let haveBlockUnlockerAward = false;
                if(global.config.blockUnlocker.reward && global.config.blockUnlocker.reward > 0){
                    const unlockerRewardPercent = global.config.blockUnlocker.reward / 100;
                    unblockerAward = roundUpPercent(block.reward * unlockerRewardPercent);
                    reward -= unblockerAward;
					haveBlockUnlockerAward = true;
                }
                ...
                if (block.workerScores) {
                    ...
                    Object.keys(block.workerScores).forEach(function (worker) {
						...
						addBlockStats(block.height,wallet,'score',block.workerScores[worker]);
						addBlockStats(block.height,wallet,'percent',percent);
						addBlockStats(block.height,wallet,'earn',workerReward);
						addBlockStats(block.height,wallet,"total",workerReward);
                        if(haveBlockUnlockerAward && worker === block.miner){
                            payments[worker] += unblockerAward;
							addBlockStats(wallet,'unlockReward',unblockerAward);
							addBlockStats(block.height,wallet,"total",unblockerAward);
                ...
                redisClient.multi(getDonationCommands).exec(function(error, replies) {
                    ...
                    for (var i in replies) {
                        ...
                        if(donation > 0) {
							...
							addBlockStats(block.height,donationAddress,"earn",donation);
							addBlockStats(block.height,worker,'donations',donation);
							addBlockStats(block.height,donationAddress,'total',donation);
							delBlockStats(block.height,worker,'total',donation);
                    ...
                    redisClient.multi(unlockedBlocksCommands).exec(function(error, replies){
						...
						const blockStatistic = unblockStatsRedis[block.height];
						const blockStatisticRedisCmd = [];
						for(var wallet in blockStatistic){
							blockStatisticRedisCmd.push(["hmset",config.coin+":block_shares:"+block.height,wallet,[
								blockStatistic.score,
								blockStatistic.percent,
								blockStatistic.earn,
								blockStatistic.donations,
								blockStatistic.unlockReward,
								blockStatistic.total,
							].join(":")]);
						}
						redisClient.multi(blockStatisticRedisCmd).exec(function(error, replies){
