        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var worker in payments){
                ...
                if(isIntegratedAddress){
                    ...
                    const redisQuery = [];
                    redisQuery.push(['hincrby', config.coin + ':workers:' + worker, 'balance', -amount]);
                    tci.fee[address] = 0;
                    if(config.payments.minerPayFee){
                        if(!config.payments.dynamicTransferFee){
                            redisQuery.push(['hincrby', config.coin + ':workers:' + worker, 'balance', -config.payments.transferFee]);
                    ...
                    redisQuery.push(['hincrby', config.coin + ':workers:' + worker, 'paid', amount]);
