        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var worker in payments){
                ...
                if(isIntegratedAddress){
                    ...
                    const redisQuery = [];
                    redisQuery.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'balance', -amount]);
                    tci.fee[address] = 0;
                    if(config.payments.minerPayFee){
                        if(!config.payments.dynamicTransferFee){
                            redisQuery.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'balance', -config.payments.transferFee]);
                    ...
                    redisQuery.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'paid', amount]);
            ...
            function executeTransfer(rpcCommand,transferCommand,cback){
                const rpcRequest = transferCmd.rpc;
                ...
                apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
                   ...
                   if(config.payments.minerPayFee){
                        if(config.payments.dynamicTransferFee){
                            ...
                            for(let d in transferCommand.rpc.destinations){
                                const destination = transferCommand.rpc.destinations[d];
                                const destWallet = destination.address;
                                const oldAmount = destination.amount;
                                const scoreAmount = oldAmount/overallAmount;
                                const newFee = transferFee * scoreAmount;
                                const newDestAmount = oldAmount - newFee;
                            ...
                            transferCommand.rpc.amount=newAmount;
                    ...
                    if(transferCommand.rpc.do_not_relay){
                        ...
                        return executeTransfer(rpcCommand,transferCommand,cback);
                    }

                    const now = (timeOffset++) + Date.now() / 1000 | 0;
                    const txHash = rresult.tx_hash_list.join("|");
                    ...
                    for (var i = 0; i < transferCmd.rpc.destinations.length; i++){
                        ...
                        if (transferCmd.rpc.payment_id){
                            wallet += config.poolServer.paymentId.addressSeparator + transferCmd.rpc.payment_id;
                        }
                        
                        transferCmd.redis.push(['zadd', config.coin + ':payments:' + wallet, now, [
                            txHash,
                            destination.amount,
                            transferCmd.fee[wallet],
                            transferCmd.rpc.mixin
                        ].join(':')]);
