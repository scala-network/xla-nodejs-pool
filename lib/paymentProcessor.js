            ...
            function executeTransfer(transferCommand,cback){
                ...
                apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
                    if (error){
                        ...
                        log('error', logSystem, 'Payments failed to send to %j', [transferCommand.rpc.destinations]);
                        cback(false);
                        return;
                    }
                   ...
                   if(config.payments.minerPayFee){
                        if(config.payments.dynamicTransferFee){
                            ...
                            log('info', logSystem, 'Dynamic Transfer Fee: %f', [transferFee]);
                            ...
                            for(let d in oldDestinations){
                                ...
                                const newFee = transferFee * scoreAmount;//if fee is 10XTL you pay 1XTL only
                                const newDestAmount = parseFloat(oldAmount) - parseFloat(newFee);//This is you new value at oldAmount - newFee
                                newDestinations.push({
                                    amount:newDestAmount,
                                    address:destWallet
                                });
                                newAmount+=parseFloat(newDestAmount);
                                newTransferFee[destWallet]=newFee;
                                log('info', logSystem, 'New(old) destinations amount: %f(%f) score %f with fee at %d', [
                                    newDestAmount,
                                    oldAmount,
                                    scoreAmount,
                                    newFee
