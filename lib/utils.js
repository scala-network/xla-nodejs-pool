...
exports.validateMinerAddress = function(address) {
    ...
    if (addressPrefix === addressBase58Prefix) {

        let addr = address.split(config.poolServer.paymentId.addressSeparator);
        if(addr.length >= 2){
            return hasValidPaymentId(addr[1]);
        }

        return true;
    }

    if(addressPrefix === integratedAddressBase58Prefix){
        return true;
    }
...
const hasValidPaymentId = function(address){
    ...
    let payment_id = cleanupSpecialChars(address);
