    ...
    async.waterfall([
        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var worker in payments){
                ...
                if(isIntegratedAddress){
                    ...
                    if(config.payments.minerPayFee){
                        ...
                        tci.redis.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'balance', -deductAmount]);
                    }

                    tci.redis.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'paid', amount]);
                ...
                apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
                   ...
                   result.fee_list.map(function(k){
                    ...
                    if(transferCommand.rpc.do_not_relay){
                        ...
                        if(config.payments.minerPayFee && config.payments.dynamicTransferFee){
                                ...
                                for(let d in oldDestinations){
                                    ...
                                    const oldAmount = parseInt(destination.amount);
                                    const scoreAmount = oldAmount/overallAmount;//10/100 = 0.1 is your score
                                    let newFee = parseInt(transferFee * scoreAmount);//if fee is 10XTL you pay 1XTL only
                                ...
                                transferCommand.rpc.do_not_relay = (transferCommand.rpc.fee !== transferFee);
                    ...
                    for (var i = 0; i < transferCommand.rpc.destinations.length; i++){
                        ...
                        //     fee:transferCommand.fee[wallet]
                        // });
                        log('info', logSystem, 'Pending transaction for payment of %s (fee: %s) to %s', [ utils.getReadableCoins(destination.amount),utils.getReadableCoins(transferCommand.fee[wallet]),wallet ]);
                    }
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                ...
                //     log('info', logSystem, 'Payment of %s (fee: %s) to %s', [ utils.getReadableCoins(notify.amount),utils.getReadableCoins(notify.fee),notify.address ]);
                // }    
