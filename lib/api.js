...
function collectStats(){
	...
	const recipients = Object.keys(config.donations);
	const donateFromIndex = redisCommands.length-1;
    if (recipients.length > 0) {
        haveDonations = true;
        for(var i = 0;i<recipients;i++){
        	redisCommands.push(['hmget', config.coin + ':workers:' + recipients[i], 'balance', 'paid']);
        }
    };
    ...
    async.parallel({
        ...
        pool: function(callback){
            redisClient.multi(redisCommands).exec(function(error, replies){
				...
				var totalDonations = (function(){
                	if(!haveDonations){
                		return {};
                	}
                	var accounts = {};

					var max = replies.length - donateFromIndex;
					for(var i =0;i<recipients.length;i++){
						var o = i+donateFromIndex;
						if(!recipients[i]){
							continue;
						}
						...
						var o1 = 0;
						if(replies[o]){
							o0 = replies[o][0]?replies[o][0]:0;
							o1 = replies[o][1]?replies[o][1]:0;
						}
						accounts[recipient] += parseInt(o0) + parseInt(o1);
					}
                	return accounts;
...
function handleSetMinerPayoutLevel(urlParts, response){
    ...
    if (ip === undefined || address === undefined || level === undefined) {
		sendData({status: 'Parameters are incomplete'});
        return;
    }

    // Do not allow wildcards in the queries.
    if (ip.indexOf('*') !== -1 || address.indexOf('*') !== -1) {
        sendData({status: 'Remove the wildcard from your miner address'});
        return;
    }

    level = parseFloat(level);
    if (isNaN(level)) {
        sendData({status: 'Your minimum payout level doesn\'t look like a number'});
        return;
    }
    ...
    if (level < minLevel) {
        sendData({status: 'The minimum payout level is ' + minLevel});
        return;
    }

    if (maxLevel && level > maxLevel) {
        sendData({status: 'The maximum payout level is ' + maxLevel});
        return;
    }
    ...
    minerSeenWithIPForAddress(address, ip, function (error, found) {
        if (!found || error) {
          sendData({status: 'We haven\'t seen that IP for your address'});
          return;
        }
        ...
        redisClient.hset(config.coin + ':workers:' + address, 'minPayoutLevel', payoutLevel, function(error, value){
            if (error){
                sendData({status: 'An error occurred when updating the value in our database'});
                return;
            }

            log('info', logSystem, 'Updated minimum payout level for ' + address + ' to: ' + payoutLevel);
            sendData({status: 'done'});
        });
    });
}
